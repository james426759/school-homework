...
<html lang="en">
  ...
  <body class="bg">
      ...
      if ($_POST['search']) {
        $data = $_POST['search'];
        $sql = "SELECT * FROM search WHERE article LIKE '%".$_POST['search']."%'";
        $result = $db->query($sql);
      }
      ?> 
      <!-- 連接資料庫 -->
    ...
    <div class="container" >
      <div align="center">
      <img  alt="" width="320px" height="150px" src="/scooter/img/logo.png"/>
      </div>
      <div align="center" style="font-weight:bold; font-size:20px; color:#ffffff; ">想當個工具人 先從修車開始
      </div>
      <form class="" role="search" action="./" method="post">
        <div class="form-group"  >
          <div class="col-xs-6 col-sm-10 box">
            <input type="text" class="form-control" placeholder="想搜尋什麼" name="search" value="<?php echo $data ?>">
          </div>
          <div class="col-xs-4 col-sm-2 box1">
            <button type="submit" class="btn btn-default" ><img width="25px" height="18px" src="/scooter/img/search.png"></button>
          </div>
        </div>
      </form>
    </div>
            ...
            <div class="col-xs-12"> 
              <div class="thumbnail" style="background-color:rgba(255%,255%,255%,0.8);">
                ...
                <div class="caption">
                  ...
                  <br>
                  <p align="center"><a href="'.$row['links'].'?id='.$row['id'].'" class="btn btn-primary" role="button">繼續閱讀</a>
                </div>
              </div>
            </div>
          </div>
